...
class WriteBack extends Module {
  val io = IO(new Bundle() {
    ...
    val vreg_write_source  = Input(UInt(2.W))
    val vec_alu_result     = Input(UInt(Parameters.VLEN))
    val vec_mem_read_data  = Input(UInt(Parameters.VLEN))
    val vreg_write_data    = Output(UInt(Parameters.VLEN))
  })
  ...
  //Vector WriteBack
  io.vreg_write_data := MuxLookup(io.vreg_write_source, io.vec_alu_result)(
    Seq(
      VRegWriteSource.VectorALU    -> io.vec_alu_result,
      VRegWriteSource.VectorMemory -> io.vec_mem_read_data
    )
  )
}
