...
class Execute extends Module {
  val io = IO(new Bundle {
    ...
    val is_vector_alu = Input(Bool())
    // CPU 層的 isVec && vec_decoder.io.is_vector_alu
    val vec_alu_op    = Input(UInt(2.W))
    // VectorDecoder 解碼出的向量 ALU 操作（ADD/MUL/MACC）
    val vreg_vs1_data = Input(UInt(Parameters.VMemDataWidth))
    val vreg_vs2_data = Input(UInt(Parameters.VMemDataWidth))
    val vreg_vd_data  = Input(UInt(Parameters.VMemDataWidth))
    // 從 VectorRegFile 讀出的三個 128-bit operands
    val vl            = Input(UInt(Parameters.DataWidth))
    // 從 VectorCSR 來的當前 vl（32-bit CSR）
    val vec_alu_result = Output(UInt(Parameters.VMemDataWidth))
  })
  ...
  val branchTarget = alu.io.result  // Branch target is computed by ALU (PC + immediate)
  ...
  //VectorALU
  val vec_alu = Module(new VectorALU)
  vec_alu.io.vs1   := io.vreg_vs1_data
  vec_alu.io.vs2   := io.vreg_vs2_data
  vec_alu.io.vd_in := io.vreg_vd_data
  vec_alu.io.op    := io.vec_alu_op
  vec_alu.io.vl    := io.vl

  io.vec_alu_result := vec_alu.io.result
}
