...
class CPU extends Module {
  ...
  val vec_decoder = Module(new VectorDecoder)
  val vec_regs    = Module(new VectorRegFile)
  val vec_csr     = Module(new VectorCSR)

  val isVec = vec_decoder.io.is_vector_instr
  ...
  regs.io.write_enable  := Mux(isVec, vec_decoder.io.is_vsetvli, id.io.reg_write_enable)
   // 向量指令時：只有 vsetvli 需要寫回純量 rd（寫入新的 vl 值）
  // 其他向量指令（vadd/vle32 等）不寫純量 RF
  regs.io.write_address := id.io.reg_write_address
  regs.io.write_data    := Mux(
    vec_decoder.io.is_vsetvli,
    vec_csr.io.vl,
    wb.io.regs_write_data
  )
  // vsetvli：把新的 vl 值（32-bit）寫回純量 rd
  // 其他指令：走純量 WB 的 MuxLookup 結果
  regs.io.read_address1 := id.io.regs_reg1_read_address
  regs.io.read_address2 := id.io.regs_reg2_read_address

  vec_regs.io.write_enable  := isVec && vec_decoder.io.vreg_write_enable
  vec_regs.io.write_address := vec_decoder.io.vd
  vec_regs.io.write_data    := wb.io.vreg_write_data
  vec_regs.io.read_address_vs1 := vec_decoder.io.vs1
  vec_regs.io.read_address_vs2 := vec_decoder.io.vs2
  vec_regs.io.read_address_vd  := vec_decoder.io.vd
  vec_regs.io.debug_read_address := io.vreg_debug_read_address
  io.vreg_debug_read_data        := vec_regs.io.debug_read_data

  regs.io.debug_read_address := io.debug_read_address
  io.debug_read_data         := regs.io.debug_read_data

  id.io.instruction := inst_fetch.io.instruction
  vec_decoder.io.instruction := inst_fetch.io.instruction

  // VectorCSR
  vec_csr.io.is_vsetvli := vec_decoder.io.is_vsetvli
  vec_csr.io.rs1_data   := regs.io.read_data1
  vec_csr.io.zimm       := vec_decoder.io.zimm
  ...
  ex.io.is_vector_alu := isVec && vec_decoder.io.is_vector_alu
  ex.io.vec_alu_op    := vec_decoder.io.valu_op
  ex.io.vreg_vs1_data := vec_regs.io.read_data_vs1
  ex.io.vreg_vs2_data := vec_regs.io.read_data_vs2
  ex.io.vreg_vd_data  := vec_regs.io.read_data_vd
  ex.io.vl            := vec_csr.io.vl

  mem.io.alu_result          := ex.io.mem_alu_result
  mem.io.reg2_data           := regs.io.read_data2
  mem.io.memory_read_enable  := Mux(isVec, false.B, id.io.memory_read_enable)
  mem.io.memory_write_enable := Mux(isVec, false.B, id.io.memory_write_enable)
  mem.io.funct3              := inst_fetch.io.instruction(14, 12)
  mem.io.is_vle32            := isVec && vec_decoder.io.is_vle32
  mem.io.is_vse32            := isVec && vec_decoder.io.is_vse32
  mem.io.vreg_store_data     := vec_regs.io.read_data_vd
  mem.io.vl                  := vec_csr.io.vl
  ...
  io.vector_memory_bundle <> mem.io.vector_memory_bundle
  ...
  wb.io.vreg_write_source    := vec_decoder.io.vreg_write_source
  wb.io.vec_alu_result       := ex.io.vec_alu_result
  wb.io.vec_mem_read_data := mem.io.wb_vector_read_data
}
